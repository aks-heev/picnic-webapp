-- 2026-08-20: recording an Airbnb booking by hand was blocked by Airbnb's own
-- iCal echo of that same reservation (sync-ical writes a source='ical' row for
-- the night within the hour, and the add path rejected any 'ical' row outright).
-- admin_edit_booking got the equivalent "skip my own footprint" fix on
-- 2026-08-15; the add path has no prior footprint to skip, so instead:
-- when the entry carries an external_booking_ref (i.e. the admin is recording a
-- booking the external platform already knows about), 'ical' rows no longer
-- block. 'admin' and 'parent' blocks still block unconditionally, and the
-- confirmed-bookings occupancy count is untouched, so a genuine double-booking
-- against our own data is still impossible.
...
declare
  ...
  -- Set when the admin is recording an externally-sourced booking (Airbnb etc).
  v_ext_ref    text := nullif(p_booking->>'external_booking_ref', '');
  d            date;
  a            jsonb;
begin
  ...
  if v_type in ('cafe', 'custom') then
    ...
    for d in select gs::date from generate_series(v_date, v_end - 1, interval '1 day') gs loop
      if exists (
        select 1 from venue_availability
        where venue_id = v_venue_id and date = d
          and ( source in ('admin', 'parent')
                or (source = 'ical' and v_ext_ref is null) )
      ) then
