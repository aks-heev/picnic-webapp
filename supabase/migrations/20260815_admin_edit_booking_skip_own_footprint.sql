-- admin_edit_booking: conflict-check ONLY the (venue, night) pairs the booking is
-- newly moving onto. Nights it already occupies are never re-checked.
--
-- Why: Airbnb re-exports blocks we publish, and sync-ical imports them back as
-- `ical` rows with booking_id = NULL. The existing `booking_id is distinct from
-- p_booking_id` filter cannot recognise those as the booking's own footprint, so a
-- stay that was manually blocked on Airbnb became permanently uneditable — even for
-- an edit that changed nothing about its dates (live case: booking #91, Umber,
-- 2026-08-15..31, "2026-08-15 is blocked (admin block, Airbnb booking, or
-- whole-floor booking)").
--
-- Safety: the relaxation can only ever apply to nights the booking ALREADY holds.
-- Any night it is moving onto is, by definition, absent from the old footprint and
-- is still fully checked. Moving a booking onto genuinely occupied dates remains
-- impossible. Verified live 2026-08-15 with three rolled-back DO-block tests:
--   1. unchanged-dates edit of #91 over NULL-booking_id ical blocks  -> SUCCEEDS
--   2. moving #91 back onto blocked 2026-08-13                       -> still raises
--   3. non-admin caller                                              -> 'Admin login required'
--
-- No signature change: still (bigint, jsonb, jsonb) — no PostgREST overload risk.
...
begin
  ...
  -- AND skip any (venue, night) this booking already occupies — see header note.
