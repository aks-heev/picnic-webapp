-- admin_add_manual_booking: persist discount_amount (signed on-site extra/discount).
-- Only change vs prior version: discount_amount added to the INSERT column list + values.
-- No signature change (still (jsonb, jsonb)) so no PostgREST overload risk.
