...
**ADDENDUM 3 — clean URLs on `picnic-dashboard` (built-unverified until the push deploys)**: files renamed to match routes via `git mv` in Aksheev's git block — `index.html` → `revenue-dashboard.html`, `hub.html` → `index.html`, `ads.html` → `meta-dashboard.html`. `hosted-dashboard/vercel.json` now has `cleanUrls: true` + temporary redirects `/hub(.html)` → `/` and `/ads(.html)` → `/meta-dashboard`. Routes: `/` = hub, `/revenue-dashboard` = the pre-existing revenue/occupancy dashboard, `/meta-dashboard` = Ad Pulse. Renamed rather than rewritten because Vercel serves an existing file before applying `rewrites`, so `/` → hub could never beat `index.html`. Old `/index.html` bookmarks now land on the hub, not the revenue page. Any doc naming `hub.html`/`ads.html` in this folder now refers to the new names.
