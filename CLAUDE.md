...
- **Still genuinely NOT-done: `notify-booking-received` (v31) renders neither field.** The guest ack has **no TIME row and no `TIME_SLOTS` constant at all**, and the admin alert shows no food/slot info — so the team cannot eyeball what was entered without opening the panel. Mirror v30's rule exactly: the booking is the only source of truth, `false` and `null` both mean render nothing. `verify_jwt=false`; preserve it.
