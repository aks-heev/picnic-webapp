...
- 🔴 **`notify-booking-received`: v32 is WRITTEN AND TESTED BUT NOT DEPLOYED — deployed is still v31.** The repo file `supabase/functions/notify-booking-received/index.ts` (602 lines) is AHEAD of the live function. This is exactly the local-vs-deployed drift §4.1 warns about; do not assume the repo reflects production here.
  - **What it adds:** a `TIME_SLOTS` constant and `formatTime()` copied verbatim from notify-booking-confirmed v28; `slotTimeText()` (booking's own start/end wins, else the slot's default window, else omit the row); `inclusionText()` mirroring v30 exactly — no venue-multiplier fallback, `false` and `null` both render nothing. TIME + INCLUDED rows added to the guest ack; Time + Includes rows added to the admin alert table.
  - **Verified before deploy:** esbuild bundle clean against stubbed `_shared`; 11 unit tests pass (override wins, slot fallback, only-start-set falls back, stay with no slot, midnight edge, singular/plural, true-but-zero, explicit false, NULL, undefined).
  - 🔴 **BLOCKER: `deploy_edge_function` cannot be called from Cowork.** Its MCP schema is an untyped `{"type":"object"}`, so the harness stringifies `verify_jwt` and `files` and the server's Zod validation rejects both (`expected boolean, received string` / `expected array, received string`). Two attempts, identical failure. Deploy via the Supabase Dashboard (Edge Functions → notify-booking-received → paste `index.ts` plus the four unchanged `_shared/*.ts`) or `supabase functions deploy notify-booking-received`. 🔴 **`verify_jwt` MUST stay false** — it is trigger-called; the wrong flag silently kills email delivery.
