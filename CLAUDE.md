...
- 🔴 **The food and slot-time fields render in NO email.** `notify-booking-confirmed` (v27) and `notify-booking-received` (v31) were never updated. Needed: a `manualInclusion(record)` helper taking precedence over `getInclusionText`, a slot-time override on the picnic TIME row, and suppression of the *"Inclusions are based on the number of adults"* footnote on the manual path (it is false there). The guest-ack email has **no TIME row and no `TIME_SLOTS` constant at all** — both must be added. Both fns are `verify_jwt=false`; preserve it.
