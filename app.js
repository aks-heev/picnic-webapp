...
// ── Admin → customer WhatsApp outreach ────────────────────────
// Pre-fills a message so the admin can review/edit it in WhatsApp before
// sending — this does not send anything itself, it only opens the chat.
function buildAdminOutreachMessage(query) {
  const venueName = query.venues?.name || query.venue_address || ''
  let dateStr = ''
  if (query.preferred_date) {
    const d = new Date(query.preferred_date + 'T00:00:00')
    dateStr = d.toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })
  }
  const slot = CAFE_SLOTS.find(s => s.key === query.time_slot)

  const lines = [
    `Hi ${query.full_name || 'there'}! This is the Picnic Stories team 🌿`,
    'Thanks for your enquiry — sharing a few details to help plan it:',
    '',
  ]
  if (venueName) lines.push(`📍 Venue: ${venueName}`)
  if (dateStr) lines.push(`📅 Date: ${dateStr}`)
  if (slot) lines.push(`⏰ Slot: ${slot.label} (${slot.time})`)
  if (query.guest_count) lines.push(`👥 Guests: ${query.guest_count}`)
  if (query.occasion) lines.push(`🎉 Occasion: ${query.occasion}`)
  lines.push('', 'Let us know if you have any questions!')
  return lines.join('\n')
}

// wa.me href for an admin-initiated chat with this lead. Returns '' when
// there's no usable 10-digit mobile number to message (guard the caller).
function queryWhatsAppHref(query) {
  const digits = String(query.mobile_number || '').replace(/\D/g, '')
  ...
  container.innerHTML = sorted.map(query => {
    ...
    <div class="adm-card adm-card--query" data-id="${escapeHtml(query.id)}">
      ...
      <div class="adm-contact-row">
        ...
        ${queryWhatsAppHref(query) ? `
        <a class="adm-contact-link adm-contact-link--wa" href="${queryWhatsAppHref(query)}" target="_blank" rel="noopener noreferrer">
